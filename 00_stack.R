...
dp$Elevation<-dp$Elevation/1000
values_on_mesh<-values_on_mesh/1000
...
temp_early$Month<-as.factor(temp_early$Month)
temp_late$Month<-as.factor(temp_late$Month)
...
formula4_mean<-Rain_mean ~ -1+Intercept+scale(Elevation):Month+Slope+scale(Aspect)+f(spatial_field, model=spde.nonstat)+
  f(spatio_temporal_field,model=spde.nonstat, group = spatio_temporal_field.group, 
    control.group = list(model="ar1"))

formula4_min<- Rain_no_days~ -1 +Intercept +scale(Elevation):Month+Slope+scale(Aspect)+f(spatial_field, model=spde.nonstat)+
  f(spatio_temporal_field,model=spde.nonstat, group = spatio_temporal_field.group, 
    control.group = list(model="ar1"))

formula4_dry_spell<-Rain_no_length~ -1 +Intercept + scale(Elevation):Month+Slope+scale(Aspect)+f(spatial_field, model=spde.nonstat)+
  f(spatio_temporal_field,model=spde.nonstat, group = spatio_temporal_field.group, 
    control.group = list(model="ar1"))
...
formula4_max_late<-inla.mdata(Rain_max, spread.x_late, tail.x_late) ~ -1+Intercept +scale(Elevation):Month+Slope+scale(Aspect)+
  f(spatial_field, model=spde.nonstat)+ f(spatio_temporal_field, model=spde.nonstat, group = spatio_temporal_field.group, 
    control.group = list(model="ar1"))
